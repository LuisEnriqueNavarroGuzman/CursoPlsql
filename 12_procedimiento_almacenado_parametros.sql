 ...
 create or replace procedure aumentasueldo(tiempoEmpresa in number, porcentaje in number)
as
 begin
 update empleados set sueldo=sueldo+(sueldo*porcentaje/100)
 where (extract(year from current_date)-extract(year from tiempoEmpresa))>tiempoEmpresa;
 end aumentasueldo;
